            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)): 
                        ...
                        target_distance = depth_data.get_distance(dep_x, dep_y)
                        cv2.circle(dm0,(dep_x, dep_y),2,(0,0,122),1,cv2.LINE_AA)

                        # 타겟의 거리가 적당한것만 (오류제외)
                        if 0.01 < target_distance < 7.0:
                            # 맨처음 키자마자 추적할놈은?
                            if following_pers == 0:
                            ...
                            if following_pers == id:
                                ...
                                distance_val = int(target_distance * 100)
                                print("{} : 타겟 person과의 거리 = {:.2f} cm".format(id, target_distance * 100))

                            # 타겟외에 다른 person
                            #else:
                            #    print(id,":person과의 거리 = {:.2f} cm".format(target_distance * 100))
